 ...
 * (`apps/web/scripts/generate-api-types.mjs`). Two clients of one API must not
 ...
 * container can predate a router that exists in `services/api/app`. Check that the
...
if (!existsSync(cli)) {
  console.error(
    'openapi-typescript is missing. Run `npm install` at the monorepo root — it is ' +
      'hoisted there rather than installed in apps/field/.',
  );
  process.exit(1);
}
