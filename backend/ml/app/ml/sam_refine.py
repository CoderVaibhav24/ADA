...
Runs on GPU when available — CUDA or Apple Metal, whichever `ml/gpu.py` picked
(~0.5 GB for hiera-small). Lazily loaded so the model is only
pulled/instantiated the first time an analysis needs it.
"""
...
def _half(device: str):
    ...
    refinement rather than an error, so it would fail silently. That argument is
    the reason this is not simply `.half()` on Metal either, where the decoder
    is if anything more prone to it.

    Thin wrapper kept for the two call sites below; the backend choice and the
    fallback when a build has no autocast for it both live in `ml/gpu.py`.
    """
    return gpu.autocast(device)
...
class Sam2Refiner(_BoxRefiner):
    ...
    def _segment_box(self, crop: np.ndarray, box: list[float]) -> np.ndarray:
        ...
        with torch.inference_mode(), _half(self.device):
