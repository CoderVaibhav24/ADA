...
class LandCoverBackend:
    ...
    tile_size = 512
    batch_size = 2      # small: GPU memory is shared with ChangeStar and SAM
    ...
    def __init__(self, repo: str) -> None:
        ...
        self.torch = torch
        # gpu.device() also installs the process-wide memory cap on first use,
        # so this model cannot grow past the budget however large the scene is —
        # and picks CUDA, Metal or CPU without this module naming one.
        self.device = gpu.device()
        # fp16 on GPU: halves the weights and activations so this coexists with
        # ChangeStar and SAM inside a laptop's budget. B5 is the reason this
        # matters — 84 M parameters is 340 MB in fp32.
    ...
    def probs(self, chips: np.ndarray) -> np.ndarray:
        ...
        host RAM, where the same expansion on the device is free and the
        transfer is of the finished result. On unified memory there is no
        transfer to save, but the interpolate itself is still an order of
        magnitude faster on the GPU than in numpy.
        """
        t = self.torch
        h, w = chips.shape[1:3]
