...
None of this applies on macOS. There is no CUDA there and no Metal execution
provider either — the Apple GPU is reached through the CoreML EP, which ships
inside the ordinary `onnxruntime` arm64 wheel and links only against system
frameworks, so there is nothing to put on a search path. `add_dll_directory`
does not exist off Windows, which is what the guard below keys on.
"""
...
def _register_cuda_dlls() -> None:
    ...
    if not hasattr(os, "add_dll_directory"):     # macOS/Linux: nothing to do
        return
    try:
        import torch
    except ImportError:
