...
def release_seg_backend() -> None:
    """Release just the building segmenter (ONNX + its provider arena).

    `gpu.free()` as well as dropping the reference: on unified memory the
    session's buffers and torch's cached blocks come out of the same pool, so
    returning only one of the two still leaves the next stage short.
    """
    global _seg_backend
    import gc
