...
Each function falls back to the original scipy path when no GPU backend is
available or GPU_IMAGE_OPS is off, so a CPU-only box behaves exactly as it did.

Backend-neutral by construction: everything here is plain torch — separable
conv2d, max_pool2d, grid_sample, median — all of which Metal implements, so the
same code runs on CUDA and on Apple Silicon. The two places that needed care
are marked: `_hypot` (Metal lacks the fused op on some builds) and the strip
sizing below, which on unified memory is bounding a share of host RAM rather
...
# Bytes of GPU memory to assume per pixel of input, used to size the strips
# below. On Apple Silicon this is bounding a slice of the same unified pool the
# working grid lives in, so the budget it divides is a share, not a second card.
...
def active() -> bool:
    """True when these ops will actually run on the GPU (CUDA or Metal)."""
    return bool(settings.gpu_image_ops) and gpu.enabled()


def _as_device_array(a: np.ndarray) -> np.ndarray:
    """Contiguous, and never float64 — Metal has no fp64 at all.

    `numpy.mean` over an integer raster returns float64, so the co-registration
    planes arrive here as doubles. CUDA accepts them (slowly); MPS raises
    `Cannot convert a MPS Tensor to float64`, every caller catches it, and the
    whole GPU path degrades to scipy with one warning per call — correct
    output, ~100x slower, easy to miss.

    Narrowing to float32 rather than refusing: everything downstream of here is
    a float32 computation already (the scipy paths cast explicitly, and the
    change filters were verified to agree with them to float32 epsilon), so the
    doubles carried precision nothing consumed.
    """
...
def _tiled(fn, arrays: list[np.ndarray], halo: int) -> np.ndarray:
    ...
    for top in range(0, h, rows):
        ...
        tensors = [torch.from_numpy(_as_device_array(a[lo:hi])).to(dev)
                   for a in arrays]
        result = fn(*tensors)
...
def _hypot(gx, gy):
    """sqrt(gx^2 + gy^2), without depending on the fused op.

    `torch.hypot` is not implemented for every dtype on every Metal build, and
    an unimplemented-op error here would drop the whole scene back to scipy —
    a correct answer at a hundredth of the speed. The explicit form is the same
    value to float32 epsilon at these magnitudes (Sobel responses of a locally
    normalised plane, order 1-100), well inside the range where hypot's
    overflow protection is the only difference.
    """
...
def robust01(diff: np.ndarray, valid: np.ndarray) -> np.ndarray:
    ...
    if active():
        try:
            ...
            dev = gpu.torch_device()
            d = torch.from_numpy(_as_device_array(diff)).to(dev)
...
def _morph(mask: np.ndarray, iterations: int, op: str) -> np.ndarray:
    ...
    if active():
        try:
            ...
            x = torch.from_numpy(np.ascontiguousarray(mask)).to(gpu.torch_device())
            x = x.float()[None, None]
...
def shift(arr: np.ndarray, offset: tuple[float, float], order: int = 1,
    ...
    if active():
        try:
            ...
            src = torch.from_numpy(_as_device_array(arr)).to(gpu.torch_device())
