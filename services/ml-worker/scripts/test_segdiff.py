...
# Release the segmenter here, exactly as services/jobs.py does between stages.
# Not optional on Apple Silicon: torch's MPS watermark counts the whole
# process's Metal allocations, so a resident onnxruntime/CoreML session (~9 GB
# for this graph) is subtracted from the torch budget and every GPU op below
# fails its first allocation and falls back to scipy — correct output, silently
# ~100x slower, which is the exact failure this script exists to notice.
