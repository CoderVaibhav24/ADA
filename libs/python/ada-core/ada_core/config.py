...
def repo_root() -> Path:
    """The repository root, found by walking up for infra/compose/docker-compose.yml.

    This used to be `parents[2]` from a single backend/app/config.py. With the
    code split across services/api, services/ml-worker and libs/python/ada-core,
    the number of levels differs per caller, so the root is located by a marker
    rather than counted. The marker is the compose file in infra/compose/ — inside a
    container the tree is flattened to /app and no marker exists, which is why
    the fallback is the current directory.
    """
    here = Path(__file__).resolve()
    for candidate in (here, *here.parents):
        if (candidate / "infra" / "compose" / "docker-compose.yml").is_file():
            return candidate
    return Path.cwd()
...
# (infra/compose/.env, beside the compose file that reads it), and a root .env is still
# honoured for a checkout that predates the move. Later entries win.
ENV_FILES = (ROOT_DIR / ".env", ROOT_DIR / "infra" / "compose" / ".env")
