...
/*
 * The machine running `infra/docker-compose.yml`, as seen from the handset:
...
/*
 * Host ports as actually published. Compose falls back to 8000/8001 when infra/.env
 * leaves BACKEND_PORT/ADA_NOTIFY_PORT blank, but infra/.env sets 8010/8011, as do the
 * portal's Vite proxy, backend/api/scripts/run_host.sh and the Keycloak redirect URIs.
 */
export const LOCAL_PORTS = {
  ...
  keycloak: 8091, // keycloak: KC_HTTP_HOST_PORT in infra/.env (compose default 8090)
} as const;
  ...
  if (config.appVariant === 'production' && problems.length > 0) {
    throw new Error(
      `ada.config.ts: refusing a production build (APP_VARIANT=production) for env '${config.env}':\n  - ` +
        problems.join('\n  - ') +
        '\nSet the real URLs in ADA_ENVS in app/ada.config.ts. Release builds refuse ' +
